...
from siamese.input_helpers import InputHelper
from siamese.siamese_network import SiameseLSTM
from siamese.siamese_network_semantic import SiameseLSTMw2v
...
with tf.Graph().as_default():
    ...
    out_dir = os.path.abspath(os.path.join("/tmp/siamese", "runs", timestamp))
    print("Writing to {}\n".format(out_dir))
    ...
    for nn in range(sum_no_of_batches*FLAGS.num_epochs):
        batch = batches.__next__()  # TODO: Replace the ``__next__()`` with a for loop
        if len(batch)<1:
            continue
